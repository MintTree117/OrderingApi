    ...
    : BaseService<AccountSecurityManager>( logger )
{
    ...
    internal async Task<Reply<List<string>>> GenerateRecoveryCodes( string userId )
    {
        var userReply = await _userManager.FindById( userId );
        if (!userReply)
            return Reply<List<string>>.UserNotFound();
        ...
        return codes is not null
            ? Reply<List<string>>.Success( codes.ToList() )
            : Reply<List<string>>.ServerError();
    }
