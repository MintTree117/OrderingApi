...
internal sealed class AccountPasswordSystem( IdentityConfigCache configCache, UserManager<UserAccount> userManager, IEmailSender emailSender )
{
    ...
    async Task<string> GenerateResetLink( UserAccount user ) =>
        $"{_configCache.ResetPasswordPage}?Email={user.Email}&Code={IdentityUtils.Encode( await _userManager.GeneratePasswordResetTokenAsync( user ) )}";
    async Task<IdentityResult> TryResetPassword( Reply<UserAccount> user, ResetPasswordRequest request ) =>
        await _userManager.ResetPasswordAsync( user.Data, IdentityUtils.Decode( request.Code ), request.NewPassword );
}