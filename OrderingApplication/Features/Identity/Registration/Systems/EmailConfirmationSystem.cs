...
namespace OrderingApplication.Features.Identity.Registration.Systems;

internal sealed class EmailConfirmationSystem( IdentityConfigCache configCache, UserManager<UserAccount> userManager, IEmailSender emailSender )
{
    readonly IdentityConfigCache _configCache = configCache;
    readonly UserManager<UserAccount> _userManager = userManager;
    readonly IEmailSender _emailSender = emailSender;

    internal async Task<Reply<bool>> SendConfirmationLink( string email )
    {
        // VALIDATE
        ...
        // SEND EMAIL
        Reply<bool> emailResult = await SendConfirmationEmail( userReply.Data );
        return emailResult;
    }
    ...
    async Task<Reply<bool>> SendConfirmationEmail( UserAccount user )
    {
        string code = IdentityValidationUtils.Encode( await _userManager.GenerateEmailConfirmationTokenAsync( user ) );
    ...
    {
        IdentityResult result = await _userManager.ConfirmEmailAsync( user, IdentityValidationUtils.Decode( code ) );
