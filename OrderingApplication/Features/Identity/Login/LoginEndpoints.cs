...
namespace OrderingApplication.Features.Identity.Login;
...
internal static class LoginEndpoints
{
    ...
    internal static void MapLoginEndpoints( this IEndpointRouteBuilder app )
    {
        ...
        app.MapGet( "api/identity/refresh", static async ( HttpContext http, LoginJwtRefresher refresher ) =>
            (await refresher.GetAccessToken( http.UserId() )) ).RequireAuthorization( Cookies );
        app.MapPut( "api/identity/logout", static async ( [FromBody] string refreshToken, HttpContext http ) =>
            await http.SignOutAsync( CookieAuthenticationDefaults.AuthenticationScheme ) ).RequireAuthorization( Cookies );
    }
    ...
    {
        Reply<bool> parseReply = IdentityTokenUtils.ParseToken( accessToken, config, out ClaimsPrincipal? claims, out JwtSecurityToken? jwt );
